...
func (l Lock) Unlock(ctx context.Context) error {
	var errs []error
	_, err := l.conn.ExecContext(ctx, fmt.Sprintf("SELECT pg_advisory_unlock(%d)", l.ID))
	if err != nil {
...
func (c Client) TryLock(ctx context.Context, id string) (*Lock, error) {
	...
	query := fmt.Sprintf("SELECT pg_try_advisory_lock(%d)", hash)
	var acquired bool
	if err := c.GetContext(ctx, &acquired, query); err != nil {
		var errs []error
		errs = append(errs, err)
