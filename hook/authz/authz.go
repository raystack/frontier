...
func (a Authz) ServeHook(res *http.Response, err error) (*http.Response, error) {
	...
	for id, attr := range config.Attributes {
		...
		case hook.AttributeTypeGRPCPayload:
			...
			payloadField, err := body_extractor.GRPCPayloadHandler{}.Extract(&res.Body, attr.Index)
			if err != nil {
		...
		case hook.AttributeTypeJSONPayload:
			...
			payloadField, err := body_extractor.JSONPayloadHandler{}.Extract(&res.Body, attr.Key)
			if err != nil {
