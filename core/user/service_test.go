...
func mockService(t *testing.T) (*mocks.Repository, *mocks.RelationService) {
	t.Helper()

	repo := mocks.NewRepository(t)
	relationService := mocks.NewRelationService(t)
	return repo, relationService
}
...
func TestService_GetByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			setup: func() *user.Service {
				repo, relationService := mockService(t)
				repo.EXPECT().GetByID(mock.Anything, testID.String()).Return(user.User{
					ID:   testID.String(),
					Name: "test",
				}, nil)
				return user.NewService(repo, relationService)
			},
		},
		{
			name: "get by email",
