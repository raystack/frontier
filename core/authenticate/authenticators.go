...
func authenticateWithPAT(ctx context.Context, s *Service) (Principal, error) {
	...
	if err != nil {
		s.log.DebugContext(ctx, "failed to get PAT owner", "err", err)
		return Principal{}, err
	}
...
func authenticateWithAccessToken(ctx context.Context, s *Service) (Principal, error) {
	...
	if genClaim, ok := insecureJWT.Get(token.GeneratedClaimKey); ok {
		...
		if !ok || claimVal != token.GeneratedClaimValue {
			s.log.DebugContext(ctx, "generated claim value mismatch")
		...
		if err != nil || !utils.IsValidUUID(userID) {
			s.log.DebugContext(ctx, "failed to parse as internal token ", "err", err)
			return Principal{}, errors.ErrUnauthenticated
		}
		...
		if claims[token.SubTypeClaimsKey] == schema.PATPrincipal {
			...
			if err != nil {
				s.log.DebugContext(ctx, "failed to get PAT", "err", err)
				return Principal{}, errors.ErrUnauthenticated
			}
...
func authenticateWithClientCredentials(ctx context.Context, s *Service) (Principal, error) {
	...
	if err != nil {
		s.log.DebugContext(ctx, "failed to decode user secret", "err", err)
		return Principal{}, errors.ErrUnauthenticated
	}
	userSecretParts := strings.Split(string(userSecret), ":")
	if len(userSecretParts) != 2 {
		s.log.DebugContext(ctx, "failed to parse user secret")
		return Principal{}, errors.ErrUnauthenticated
	}
	...
	s.log.DebugContext(ctx, "failed to authenticate with client credentials", "err", err)
	return Principal{}, errors.ErrUnauthenticated
}
