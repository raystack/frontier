...
func (h *ConnectHandler) ListSubscriptions(ctx context.Context, request *connect.Request[frontierv1beta1.ListSubscriptionsRequest]) (*connect.Response[frontierv1beta1.ListSubscriptionsResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(ctx, fmt.Errorf("ListSubscriptions.GetByOrgID: org_id=%s: %w", request.Msg.GetOrgId(), err))
	}
	billingID := cust.ID
	...
	if err != nil {
		return nil, mapBillingError(ctx, fmt.Errorf("ListSubscriptions.List: billing_id=%s org_id=%s state=%s plan_id=%s: %w",
	...
	for _, v := range subscriptionList {
		...
		if err != nil {
			return nil, mapBillingError(ctx, fmt.Errorf("ListSubscriptions: entity_id=%s: %w", v.ID, err))
		}
...
func (h *ConnectHandler) GetSubscription(ctx context.Context, request *connect.Request[frontierv1beta1.GetSubscriptionRequest]) (*connect.Response[frontierv1beta1.GetSubscriptionResponse], error) {
	...
	if err != nil {
		return nil, mapBillingError(ctx, fmt.Errorf("GetSubscription.GetByID: subscription_id=%s: %w", request.Msg.GetId(), err))
	}

	subscriptionPB, err := transformSubscriptionToPB(subscription)
	if err != nil {
		return nil, mapBillingError(ctx, fmt.Errorf("GetSubscription: entity_id=%s: %w", subscription.ID, err))
	}
...
func (h *ConnectHandler) CancelSubscription(ctx context.Context, request *connect.Request[frontierv1beta1.CancelSubscriptionRequest]) (*connect.Response[frontierv1beta1.CancelSubscriptionResponse], error) {
	...
	if err != nil {
		return nil, mapBillingError(ctx, fmt.Errorf("CancelSubscription.Cancel: subscription_id=%s immediate=%v: %w", request.Msg.GetId(), request.Msg.GetImmediate(), err))
	}
...
func (h *ConnectHandler) ChangeSubscription(ctx context.Context, request *connect.Request[frontierv1beta1.ChangeSubscriptionRequest]) (*connect.Response[frontierv1beta1.ChangeSubscriptionResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(ctx, fmt.Errorf("ChangeSubscription.ChangePlan: subscription_id=%s plan_id=%s immediate=%v cancel_upcoming=%v: %w",
