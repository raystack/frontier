...
func (h *ConnectHandler) CreateBillingUsage(ctx context.Context, request *connect.Request[frontierv1beta1.CreateBillingUsageRequest]) (*connect.Response[frontierv1beta1.CreateBillingUsageResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(fmt.Errorf("CreateBillingUsage.GetByOrgID: org_id=%s: %w", request.Msg.GetOrgId(), err))
	}
	...
	if err := h.usageService.Report(ctx, createRequests); err != nil {
		...
		return nil, mapBillingError(fmt.Errorf("CreateBillingUsage.Report: billing_id=%s org_id=%s usage_count=%d: %w",
...
func (h *ConnectHandler) ListBillingTransactions(ctx context.Context, request *connect.Request[frontierv1beta1.ListBillingTransactionsRequest]) (*connect.Response[frontierv1beta1.ListBillingTransactionsResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(fmt.Errorf("ListBillingTransactions.GetByOrgID: org_id=%s: %w", request.Msg.GetOrgId(), err))
	}
	billingID := cust.ID
	...
	if err != nil {
		return nil, mapBillingError(fmt.Errorf("ListBillingTransactions.List: org_id=%s billing_id=%s start_range=%v end_range=%v: %w",
	...
	for _, v := range transactionsList {
		...
		if err != nil {
			return nil, mapBillingError(fmt.Errorf("ListBillingTransactions: entity_id=%s: %w", v.ID, err))
		}
...
func (h *ConnectHandler) TotalDebitedTransactions(ctx context.Context, request *connect.Request[frontierv1beta1.TotalDebitedTransactionsRequest]) (*connect.Response[frontierv1beta1.TotalDebitedTransactionsResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(fmt.Errorf("TotalDebitedTransactions.GetByOrgID: org_id=%s: %w", request.Msg.GetOrgId(), err))
	}
	billingID := cust.ID

	debitAmount, err := h.creditService.GetTotalDebitedAmount(ctx, billingID)
	if err != nil {
		return nil, mapBillingError(fmt.Errorf("TotalDebitedTransactions.GetTotalDebitedAmount: org_id=%s billing_id=%s: %w",
...
func (h *ConnectHandler) RevertBillingUsage(ctx context.Context, request *connect.Request[frontierv1beta1.RevertBillingUsageRequest]) (*connect.Response[frontierv1beta1.RevertBillingUsageResponse], error) {
	...
	if err != nil {
		...
		return nil, mapBillingError(fmt.Errorf("RevertBillingUsage.GetByOrgID: org_id=%s: %w", request.Msg.GetOrgId(), err))
	}
	...
	if err := h.usageService.Revert(ctx, cust.ID,
		...
		return nil, mapBillingError(fmt.Errorf("RevertBillingUsage.Revert: billing_id=%s org_id=%s usage_id=%s amount=%d: %w",
