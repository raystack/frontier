	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService) {
				ps.EXPECT().List(mock.Anything, project.Filter{Principal: &authenticate.Principal{ID: "user-1", Type: schema.UserPrincipal}}).Return([]project.Project{
					{
						ID:    "project-1",
						Name:  "test-project-1",
		...
		},
		{
			...
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService) {
				ps.EXPECT().List(mock.Anything, project.Filter{Principal: &authenticate.Principal{ID: "user-1", Type: schema.UserPrincipal}}).Return([]project.Project{}, nil)
			},
			req: &frontierv1beta1.ListProjectsByUserRequest{Id: "user-1"},
			want: &frontierv1beta1.ListProjectsByUserResponse{
				Projects: []*frontierv1beta1.Project{},
		...
		},
		{
			...
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService) {
				ps.EXPECT().List(mock.Anything, project.Filter{Principal: &authenticate.Principal{ID: "non-existent-user", Type: schema.UserPrincipal}}).Return(nil, user.ErrNotExist)
			},
		...
		},
		{
			...
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService) {
				ps.EXPECT().List(mock.Anything, project.Filter{Principal: &authenticate.Principal{ID: "user-1", Type: schema.UserPrincipal}}).Return(nil, errors.New("database error"))
			},
			req:  &frontierv1beta1.ListProjectsByUserRequest{Id: "user-1"},
			want: nil,
			err:  connect.CodeInternal,
...
func TestConnectHandler_ListProjectsByCurrentUser(t *testing.T) {
	tests := []struct {
		title string
		setup func(*mocks.ProjectService, *mocks.AuthnService, *mocks.ResourceService)
		req   *frontierv1beta1.ListProjectsByCurrentUserRequest
		want  *frontierv1beta1.ListProjectsByCurrentUserResponse
		err   connect.Code
	}{
		{
			title: "should list current user projects successfully",
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService, rs *mocks.ResourceService) {
				mockPrincipal := authenticate.Principal{
					ID:   "user-1",
					Type: "app/user",
					User: &user.User{ID: "user-1", Email: "test@example.com"},
				...
				ps.EXPECT().List(mock.Anything, mock.MatchedBy(func(filter project.Filter) bool {
					if filter.Principal == nil || *filter.Principal != mockPrincipal {
						return false
					}
					return filter.OrgID == ""
			...
			want: &frontierv1beta1.ListProjectsByCurrentUserResponse{
				...
				AccessPairs: []*frontierv1beta1.ListProjectsByCurrentUserResponse_AccessPair{},
			},
			err: connect.Code(0),
		},
		{
			title: "should list current user projects with organization filter",
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService, rs *mocks.ResourceService) {
				mockPrincipal := authenticate.Principal{
					ID:   "user-1",
					Type: "app/user",
			...
			req: &frontierv1beta1.ListProjectsByCurrentUserRequest{OrgId: "org-1"},
			want: &frontierv1beta1.ListProjectsByCurrentUserResponse{
				Projects: []*frontierv1beta1.Project{
					{
						Id:           "project-1",
		...
		{
			title: "should return empty list when current user has no projects",
			setup: func(ps *mocks.ProjectService, as *mocks.AuthnService, rs *mocks.ResourceService) {
				mockPrincipal := authenticate.Principal{
					ID:   "user-1",
					Type: "app/user",
	...
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			...
			if tt.setup != nil {
				tt.setup(mockProjectSrv, mockAuthnSrv, mockResourceSrv)
			}
