...
// membersFromUsers converts a user slice to membership.Member slice for mocking
// membershipService.ListPrincipalsByResource results.
func membersFromUsers(users []user.User, roles ...role.Role) []membership.Member {
...
func userIDsOf(users []user.User) []string {
...
func TestConnectHandler_UpdateGroup(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(gs *mocks.GroupService, ms *mocks.MetaSchemaService, os *mocks.OrganizationService) {
				ms.EXPECT().Validate(mock.AnythingOfType("metadata.Metadata"), groupMetaSchema).Return(nil)
				gs.EXPECT().Get(mock.Anything, "some-id").Return(group.Group{}, group.ErrInvalidUUID)
			},
			request: connect.NewRequest(&frontierv1beta1.UpdateGroupRequest{
				Id: "some-id",
				Body: &frontierv1beta1.GroupRequestBody{
		...
		{
			name: "should return not found error if group service update returns not exist",
			setup: func(gs *mocks.GroupService, ms *mocks.MetaSchemaService, os *mocks.OrganizationService) {
				ms.EXPECT().Validate(mock.AnythingOfType("metadata.Metadata"), groupMetaSchema).Return(nil)
				gs.EXPECT().Get(mock.Anything, someGroupID).Return(group.Group{
					ID:             someGroupID,
					OrganizationID: testOrgID,
				}, nil)
				os.EXPECT().Get(mock.Anything, testOrgID).Return(testOrgMap[testOrgID], nil)
...
func TestConnectHandler_ListOrganizationGroups(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(gs *mocks.GroupService, os *mocks.OrganizationService, us *mocks.UserService, ms *mocks.MembershipService) {
				...
				ms.EXPECT().ListPrincipalsByResource(mock.Anything, testGroupID, schema.GroupNamespace, membership.MemberFilter{
					PrincipalType: schema.UserPrincipal,
				}).Return([]membership.Member{
					{PrincipalID: testUserID, PrincipalType: schema.UserPrincipal},
				}, nil)
				us.EXPECT().GetByIDs(mock.Anything, []string{testUserID}).Return([]user.User{
					{
						ID:       testUserID,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.setup != nil {
				tt.setup(mockGroupSvc, mockOrgSvc, mockUserSvc, mockMembershipSvc)
			}
			h := &ConnectHandler{
				groupService:      mockGroupSvc,
				orgService:        mockOrgSvc,
...
func TestConnectHandler_ListGroupUsers(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(gs *mocks.GroupService, us *mocks.UserService, os *mocks.OrganizationService, ms *mocks.MembershipService) {
				...
				ms.EXPECT().ListPrincipalsByResource(mock.Anything, someGroupID, schema.GroupNamespace, membership.MemberFilter{
					PrincipalType: schema.UserPrincipal,
				}).Return(membersFromUsers(testUserList), nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMembershipSvc := new(mocks.MembershipService)
			if tt.setup != nil {
				tt.setup(mockGroupSvc, mockUserSvc, mockOrgSvc, mockMembershipSvc)
			}
			h := ConnectHandler{
				groupService:      mockGroupSvc,
				userService:       mockUserSvc,
...
func TestConnectHandler_RemoveGroupUser(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockMembershipSvc := new(mocks.MembershipService)
			if tt.setup != nil {
				tt.setup(mockGroupSvc, mockOrgSvc, mockMembershipSvc)
			}
			h := ConnectHandler{
				groupService:      mockGroupSvc,
				orgService:        mockOrgSvc,
...
func TestConnectHandler_DeleteGroup(t *testing.T) {
	...
	tests := []struct {
		name    string
		setup   func(ds *mocks.CascadeDeleter)
...
func TestConnectHandler_SetGroupMemberRole(t *testing.T) {
	someGroupID := utils.NewString()
	somePrincipalID := utils.NewString()
	someRoleID := utils.NewString()

	baseRequest := func() *connect.Request[frontierv1beta1.SetGroupMemberRoleRequest] {
		return connect.NewRequest(&frontierv1beta1.SetGroupMemberRoleRequest{
			GroupId:       someGroupID,
	...
	tests := []struct {
		...
		request *connect.Request[frontierv1beta1.SetGroupMemberRoleRequest]
		want    *connect.Response[frontierv1beta1.SetGroupMemberRoleResponse]
		wantErr error
	}{
		{
		...
		},
		{
			...
			request: baseRequest(),
			wantErr: connect.NewError(connect.CodeInvalidArgument, ErrInvalidGroupRole),
		},
		{
			name: "should return invalid argument if principal type is unsupported",
			setup: func(gs *mocks.GroupService, ms *mocks.MembershipService, os *mocks.OrganizationService) {
		...
		},
		{
			...
			request: baseRequest(),
			want:    connect.NewResponse(&frontierv1beta1.SetGroupMemberRoleResponse{}),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockOrgSvc := new(mocks.OrganizationService)
			if tt.setup != nil {
				tt.setup(mockGroupSvc, mockMembershipSvc, mockOrgSvc)
			}
			h := ConnectHandler{
				groupService:      mockGroupSvc,
				membershipService: mockMembershipSvc,
			...
			got, err := h.SetGroupMemberRole(context.Background(), tt.request)
			if tt.wantErr != nil {
				assert.Error(t, err)
				assert.Equal(t, tt.wantErr.(*connect.Error).Code(), err.(*connect.Error).Code())
				assert.Contains(t, err.(*connect.Error).Message(), tt.wantErr.(*connect.Error).Message())
