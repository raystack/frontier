...
func TestOrgUsersRepository_PrepareDataQuery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:    `SELECT "policies"."resource_id" AS "org_id", "users"."id" AS "id", "users"."name" AS "name", "users"."title" AS "title", "users"."email" AS "email", "users"."state" AS "state", "users"."avatar" AS "avatar", MIN("policies"."created_at") AS "org_joined_at", STRING_AGG("roles"."name", ', ') AS "role_names", STRING_AGG(COALESCE("roles"."title", ''), ', ') AS "role_titles", STRING_AGG(CAST("roles"."id" AS TEXT), ', ') AS "role_ids" FROM "policies" INNER JOIN "users" ON ("users"."id" = "policies"."principal_id") LEFT JOIN "roles" ON ("roles"."id" = "policies"."role_id") WHERE (("policies"."resource_id" = $1) AND ("policies"."resource_type" = $2) AND ("policies"."principal_type" = $3) AND ("users"."deleted_at" IS NULL) AND ("roles"."deleted_at" IS NULL)) GROUP BY "policies"."resource_id", "users"."id", "users"."name", "users"."title", "users"."email", "users"."state", "users"."created_at", "users"."updated_at" LIMIT $4`,
		...
		},
		{
			...
			wantSQL:    `SELECT "policies"."resource_id" AS "org_id", "users"."id" AS "id", "users"."name" AS "name", "users"."title" AS "title", "users"."email" AS "email", "users"."state" AS "state", "users"."avatar" AS "avatar", MIN("policies"."created_at") AS "org_joined_at", STRING_AGG("roles"."name", ', ') AS "role_names", STRING_AGG(COALESCE("roles"."title", ''), ', ') AS "role_titles", STRING_AGG(CAST("roles"."id" AS TEXT), ', ') AS "role_ids" FROM "policies" INNER JOIN "users" ON ("users"."id" = "policies"."principal_id") LEFT JOIN "roles" ON ("roles"."id" = "policies"."role_id") WHERE (("policies"."resource_id" = $1) AND ("policies"."resource_type" = $2) AND ("policies"."principal_type" = $3) AND ("users"."deleted_at" IS NULL) AND ("roles"."deleted_at" IS NULL) AND ("users"."email" = $4)) GROUP BY "policies"."resource_id", "users"."id", "users"."name", "users"."title", "users"."email", "users"."state", "users"."created_at", "users"."updated_at" LIMIT $5`,
		...
		},
		{
			...
			wantSQL:    `SELECT "policies"."resource_id" AS "org_id", "users"."id" AS "id", "users"."name" AS "name", "users"."title" AS "title", "users"."email" AS "email", "users"."state" AS "state", "users"."avatar" AS "avatar", MIN("policies"."created_at") AS "org_joined_at", STRING_AGG("roles"."name", ', ') AS "role_names", STRING_AGG(COALESCE("roles"."title", ''), ', ') AS "role_titles", STRING_AGG(CAST("roles"."id" AS TEXT), ', ') AS "role_ids" FROM "policies" INNER JOIN "users" ON ("users"."id" = "policies"."principal_id") LEFT JOIN "roles" ON ("roles"."id" = "policies"."role_id") WHERE (("policies"."resource_id" = $1) AND ("policies"."resource_type" = $2) AND ("policies"."principal_type" = $3) AND ("users"."deleted_at" IS NULL) AND ("roles"."deleted_at" IS NULL) AND EXISTS (SELECT 1 FROM "policies" INNER JOIN "roles" ON ("roles"."id" = "policies"."role_id") WHERE (("policies"."principal_id" = "users"."id") AND ("policies"."resource_id" = $4) AND ("policies"."resource_type" = $5) AND ("roles"."name" = $6)) LIMIT $7)) GROUP BY "policies"."resource_id", "users"."id", "users"."name", "users"."title", "users"."email", "users"."state", "users"."created_at", "users"."updated_at" LIMIT $8`,
		...
		},
		{
			...
			wantSQL:    `SELECT "policies"."resource_id" AS "org_id", "users"."id" AS "id", "users"."name" AS "name", "users"."title" AS "title", "users"."email" AS "email", "users"."state" AS "state", "users"."avatar" AS "avatar", MIN("policies"."created_at") AS "org_joined_at", STRING_AGG("roles"."name", ', ') AS "role_names", STRING_AGG(COALESCE("roles"."title", ''), ', ') AS "role_titles", STRING_AGG(CAST("roles"."id" AS TEXT), ', ') AS "role_ids" FROM "policies" INNER JOIN "users" ON ("users"."id" = "policies"."principal_id") LEFT JOIN "roles" ON ("roles"."id" = "policies"."role_id") WHERE (("policies"."resource_id" = $1) AND ("policies"."resource_type" = $2) AND ("policies"."principal_type" = $3) AND ("users"."deleted_at" IS NULL) AND ("roles"."deleted_at" IS NULL) AND ((CAST("users"."name" AS TEXT) ILIKE $4) OR (CAST("users"."title" AS TEXT) ILIKE $5) OR (CAST("users"."email" AS TEXT) ILIKE $6) OR (CAST("users"."state" AS TEXT) ILIKE $7) OR (CAST("policies"."created_at" AS TEXT) ILIKE $8) OR (CAST("roles"."name" AS TEXT) ILIKE $9) OR (CAST("roles"."title" AS TEXT) ILIKE $10))) GROUP BY "policies"."resource_id", "users"."id", "users"."name", "users"."title", "users"."email", "users"."state", "users"."created_at", "users"."updated_at" LIMIT $11`,
			wantParams: []interface{}{"org123", "app/organization", "app/user", "%john%", "%john%", "%john%", "%john%", "%john%", "%john%", "%john%", int64(10)},
		},
		{
