...
func TestProjectUsersRepository_PrepareDataQuery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  `SELECT "users"."id", "users"."name", "users"."email", "users"."title", "users"."avatar", "users"."state", "policies"."resource_id" AS "project_id", MIN("policies"."created_at") AS "project_joined_at", string_agg(DISTINCT roles.name, ',') AS "role_names", string_agg(DISTINCT roles.title, ',') AS "role_titles", string_agg(DISTINCT roles.id::text, ',') AS "role_ids" FROM "policies" INNER JOIN "users" ON ("policies"."principal_id" = "users"."id") INNER JOIN "roles" ON ("policies"."role_id" = "roles"."id") INNER JOIN "projects" ON ("policies"."resource_id" = "projects"."id") WHERE (("policies"."deleted_at" IS NULL) AND (("policies"."principal_type" = $1) AND ("policies"."resource_id" = $2) AND ("policies"."resource_type" = $3)) AND ("users"."deleted_at" IS NULL) AND ("roles"."deleted_at" IS NULL) AND ("projects"."deleted_at" IS NULL)) GROUP BY "users"."id", "users"."name", "users"."email", "users"."title", "users"."state", "policies"."resource_id" LIMIT $4`,
