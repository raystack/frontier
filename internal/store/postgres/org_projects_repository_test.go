...
func TestOrgProjectsRepository_prepareDataQuery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantSQL:  `SELECT "projects"."id", "projects"."name", "projects"."title", "projects"."state", "projects"."created_at", "projects"."org_id", COUNT(DISTINCT("policies"."principal_id")) AS "member_count", array_agg(DISTINCT users.id) AS "user_ids" FROM "policies" INNER JOIN "projects" ON ("policies"."resource_id" = "projects"."id") INNER JOIN "users" ON ("policies"."principal_id" = "users"."id") WHERE (("policies"."deleted_at" IS NULL) AND (("principal_type" = $1) AND ("projects"."org_id" = $2)) AND ("projects"."deleted_at" IS NULL) AND ("users"."deleted_at" IS NULL)) GROUP BY "projects"."id", "projects"."name", "projects"."title", "projects"."state", "projects"."created_at", "projects"."org_id" LIMIT $3`,
