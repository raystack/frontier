...
- Treat `user/project` as a stand-in for `app/project` that you are allowed to write to from
  config. The name is your choice — any non-`app/*` namespace works; `user/project` is just the
  example used here.
...
permissions the role already had, or it will lose them. For a built-in role, "already had" means
the default permissions Frontier ships it with — its entry in the predefined role list
(`PredefinedRoles` in `internal/bootstrap/schema/schema.go`), not the base schema. In the example,
`app/project:get` is kept so the role can still open the project.
