...
- The product name is the identity and must be at least three characters. `title`,
  `description`, `config`, `prices`, and `features` are the managed fields.
- `title`, `description`, and `config` state the whole desired value. The file writes them
  as given, so leaving one out resets it (an empty title, an empty description, a zero
  `credit_amount`). Keep a `title` set, since the payment provider needs a product name.
  `behavior` is set only when the product is created and is not changed afterward. Metadata is
  written only when a product is created and is otherwise left as is.
- The valid values for `behavior`, `interval`, `usage_type`, and `billing_scheme` are checked
  against the API's own rules during the dry run, so a wrong value fails the plan rather than
  the apply. This kind does not keep its own copy of those lists, so it stays in step with the
  server as the lists change.
...
- Export writes each product sorted by name, and each product's active prices sorted by name,
  so reconciling an export plans nothing. Metadata is written only when a product is created.
  Provider ids, timestamps, and price state are server-owned and never written.
