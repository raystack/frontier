...
- `title`, `description`, and `config` state the whole desired value. The file writes them as
  ...
  only when the product is created and is not changed afterward. `metadata` is set only when a
  product is created; it is not managed on update and is not written by export.
...
- Export writes each product sorted by name, and each product's active prices sorted by name,
  so reconciling an export plans nothing. Provider ids, timestamps, price state, and metadata
  are not written: the first three are server-owned, and metadata is not managed on update.
