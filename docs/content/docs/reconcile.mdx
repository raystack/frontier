...
- `title`, `description`, and `config` state the whole desired value. The file writes them as
  ...
  fails the plan. Metadata is out of scope for this kind: it is never set, changed, or exported
  here.
...
- Every product on the server must appear in the file. A product that is missing fails the
  ...
  price, or has a name shorter than three characters. That product is out of scope: it is left
  untouched, is not required in the file, and is not exported.
