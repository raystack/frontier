...
- The product name is the identity. `title`, `description`, `behavior`, `config`, `prices`,
  and `features` are the managed fields.
- Prices are keyed by their name within the product. A price name the product does not have
  yet is created, and an active price the file no longer lists is retired (marked inactive,
  not deleted, because a provider price cannot be removed). Amount, currency, and interval
  cannot change once a price exists: to change an amount, add a new price under a new name,
  and the old one is retired when you drop it from the file.
...
- Export writes the active prices of each product, sorted by name, so reconciling an export
  plans nothing. Provider ids, timestamps, and price state are server-owned and never written.
