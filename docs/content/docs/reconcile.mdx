...
- The product name is the identity. `title`, `description`, `config`, `prices`, and
  `features` are the managed fields.
- The update API merges rather than overwrites, so a field you leave out keeps its current
  value: `title`, `description`, and metadata are kept when empty, and `credit_amount` and
  `seat_limit` are kept when zero. This kind can set those fields but cannot clear them
  through the file. `behavior` is set only when the product is created and is not changed
  afterward.
- Prices are keyed by their name within the product. A new price name is added, and an active
  ...
  price cannot be removed). Amount, currency, interval, and the other pricing fields cannot
...
- Every product on the server must appear in the file. A product that is missing fails the
  ...
  by hand.
- Export writes the active prices of each product, sorted by name, so reconciling an export
  plans nothing. Metadata is written only when a product is created. Provider ids, timestamps,
  and price state are server-owned and never written.
